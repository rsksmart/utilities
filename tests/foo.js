...
var contract = contracts.compile('foo.sol:Foo', 'foo.sol');
...
async()
    ...
    .then(function (data, next) {
        ...
        commands.callTransaction(host, argv.from, contract.address, 0, { data: contracts.encodeCall(contract, 'isWorking()', []) }, next);
    })
    .then(function (data, next) {
        console.log('value', utils.decodeValue(data));
        commands.processTransaction(host, argv.from, contract.address, 0, { gas: 1000000, data: contracts.encodeCall(contract, 'setWorking(bool)', [ 0 ]) }, next);
    })
    .then(function (data, next) {
        commands.callTransaction(host, argv.from, contract.address, 0, { data: contracts.encodeCall(contract, 'isWorking()', []) }, next);
    })
