...
function invokeTransfer(cb) {
    ...
    .then(function (data, next) {
        commands.processTransaction(host, argv.from, contract.address, 0, { gas: 3000000, data: contracts.encodeCall(contract, 'transfer(address,uint256)', [ address, 1 ]) }, next);
    })
    .then(function (data, next) {
    ...
    .then(function (data, next) {
        console.log('value', utils.decodeValue(data));
        commands.processTransaction(host, argv.from, contract.address, 0, { gas: 3000000, data: contracts.encodeCall(contract, 'borrow(address)', [ address ]) }, next);
    })
    .then(function (data, next) {
        commands.callTransaction(host, argv.from, contract.address, 0, { data: contracts.encodeCall(contract, 'balances(address)', [ address ]) }, next);
    })
